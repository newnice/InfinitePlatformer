...
public class PlayerCharacter : MonoBehaviour {
    ...
    void Update() {
        ...
        up = up > 0 ? up : 0;
        rb.position += new Vector2(Input.GetAxis("Horizontal") * 0.2f, up);

        distanceScore = Mathf.Max(distanceScore, (int) this.transform.position.x);
        int totalScore = distanceScore * GameplayConstants.SCORE_DISTANCE_MULTIPLIER +
                         enemyScore * GameplayConstants.SCORE_ENEMY_MULTIPLIER;
    }
