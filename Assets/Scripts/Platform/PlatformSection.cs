...
public class PlatformSection : MonoBehaviour {
    ...
    private void CalculateSpawnPoints(BoxCollider2D[] colliders) {
        ...
        foreach (BoxCollider2D col in colliders) {
            ...
            Vector2 colTop = (Vector2) col.bounds.center +
                             new Vector2(0f, col.size.y + GameplayConstants.SLIP_ZONE_WIDTH);
            ...
            Vector3 spawnCenter = colTop + new Vector2(0.5f * (rightRange - leftRange),
                                      GameplayConstants.SPAWN_ZONE_DROP_HEIGHT - GameplayConstants.SLIP_ZONE_WIDTH);
    ...
    void OnTriggerEnter2D(Collider2D coll) {
        if (nextPlatformActivated || !coll.CompareTag(TagNames.PLAYER)) return;
